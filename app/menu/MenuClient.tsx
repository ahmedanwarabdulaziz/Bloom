        ...
        <motion.div layout className="mt-1.5 flex flex-col items-start w-full relative">
            ...
            {(isTruncated || isExpanded) && (
                <motion.button 
                    ...
                    className="text-accent-main text-[10px] font-medium mt-1 hover:underline active:scale-95 inline-block py-0.5"
                >
...
export default function MenuClient({ categories, items }: MenuClientProps) {
    const [activeMainCategory, setActiveMainCategory] = useState<'food' | 'drinks'>('food');
    const [activeCategory, setActiveCategory] = useState<string>("ALL");
    const [searchTerm, setSearchTerm] = useState("");
    const [isSearchExpanded, setIsSearchExpanded] = useState(false);
    const deferredSearchTerm = useDeferredValue(searchTerm);

    const visibleCategories = categories.filter(c => (c.mainCategory || 'food') === activeMainCategory);
    ...
    const activeItems = searchFilteredItems.filter(item => {
        if (activeCategory === "ALL") {
            return visibleCategoryIds.includes(item.categoryId);
        }
                    ...
                    <div>
                            ...
                            <div className="flex flex-col items-end shrink-0">
                                {item.sizes && item.sizes.length > 0 ? (
                                    <>
                                        ...
                                        <span className="text-accent-main font-bold text-lg leading-none">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    {item.sizes.map((s, idx) => (
                                        <div key={idx} className="bg-surface-elevated border border-border rounded-control p-1.5 flex flex-col items-center justify-center text-center">
                                            ...
                                            <span className="text-accent-main font-bold text-xs leading-none mt-0.5">${s.price.toFixed(2)}</span>
                                            {/* Variation Details */}
                                            {s.servingDetails && (
                        ...
                        {item.addons && item.addons.length > 0 && (
                            <div className="mt-2 border-t border-border pt-2">
                                <p className="text-[10px] uppercase tracking-wider text-accent-main/80 font-bold mb-1">Extras</p>
                                <div className="flex flex-col gap-1.5">
                                    {item.addons.map((a, idx) => (
                                        <div key={idx} className="flex justify-between items-start text-[10px] bg-surface px-2 py-1 rounded-sm border border-border">
                                            ...
                                            <span className="text-accent-main font-medium whitespace-nowrap shrink-0 mt-0.5">+ ${a.price.toFixed(2)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
        ...
        <main className="flex-1 w-full relative flex flex-col min-h-screen bg-background">
                ...
                <div className="sticky top-0 z-50 bg-background/95 backdrop-blur-xl border-b border-border shadow-sm">
                        ...
                        <div className="flex items-center justify-between z-10 bg-background/0">
                            ...
                            <button
                                ...
                                className={`h-10 w-10 flex-shrink-0 rounded-control border flex items-center justify-center transition-all duration-300 active:scale-95 ${
                                    isSearchExpanded 
                                        ? 'bg-accent-main text-background border-accent-main shadow-md' 
                                ...
                                >
                                    <div className="relative w-full pb-1">
                                        ...
                                        <input
                                            type="text"
                                            autoFocus
                                            className="w-full bg-surface border border-border rounded-full py-2.5 pl-10 pr-10 text-main-text text-sm focus:outline-none focus:border-accent-main focus:ring-1 focus:ring-accent-main transition-colors placeholder:text-muted-text"
                    ...
                    <div className="w-full max-w-7xl mx-auto px-4 pt-3 flex gap-2 justify-center">
                        <button
                            ...
                            className={`flex-1 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95 ${
                                activeMainCategory === 'food'
                                    ? 'bg-accent-main text-background shadow-md'
                        ...
                        <button
                            onClick={() => {
                                setActiveMainCategory('drinks');
                                setActiveCategory('ALL');
                            }}
                            className={`flex-1 py-2.5 rounded-full text-sm font-bold transition-all active:scale-95 ${
                                activeMainCategory === 'drinks'
                                    ? 'bg-accent-main text-background shadow-md'
                                    : 'bg-surface text-muted-text border border-border hover:bg-surface-elevated'
                            }`}
                        >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 pt-4">
                                    ...
                                    {activeItems.length === 0 && (
                                        <div className="text-center py-20 bg-surface rounded-card border border-border mt-4">
                                            ...
                                            <h3 className="text-main-text text-lg font-medium">No items found in {visibleCategories.find(c => c.id === activeCategory)?.name}</h3>
                                            <p className="text-muted-text text-sm mt-1">Try searching for something else</p>
                                        </div>
                                    )}
                                </div>
